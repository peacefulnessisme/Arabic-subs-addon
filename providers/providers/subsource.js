...
/**
 * يبحث عن الفيلم/المسلسل في SubSource بالاسم ويطابق نتيجة IMDB ID
 */
async function findMovieId(imdbId, type, apiKey) {
  ...
  // بنية الاستجابة قد تختلف (data / results / movies) - نتحقق من كل الاحتمالات
  const list = data.results || data.data || data.movies || [];
  if (!Array.isArray(list) || list.length === 0) return null;

  // نحاول نطابق imdb_id أولاً، وإلا ناخذ أول نتيجة
...
/**
 * يجيب ترجمات عربية من SubSource
 */
